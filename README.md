...
|webアンケートシステム改修||
|営業支援ツール作成|営業の|
|集計ツール作成|CP環境にてサーバーレスアーキテクチャを採用したツール構築を実施しました。Slackや、HubSpotなどのAPIとの連携も行いました。|
|書籍管理WEBアプリ構築（OJT）|上長及び先輩社員からレビューを受けながら、AWS上でサーバーレスアーキテクチャを採用し、設計～デプロイ～テスト～運用までを行いました。|
|ブログアプリ構築（OJT）|OJTにて、SPA/REST APIの構成にてブログアプリを作成しました。|
#### プロジェクト詳細
